...
/**
 * Nav order is fixed by the spec. Items marked adminOnly are hidden from staff
 * entirely — not shown-and-disabled, so a staff account never sees a door it
 * cannot open.
 */
export const NAV = [
    ...
    { to: '/products', label: 'Products', icon: 'box', tour: 'The snacks and drinks you sell, with the cost price behind each one so profit reports stay honest.' },
    { to: '/bookings', label: 'Bookings', icon: 'calendar', tour: 'Reserve a station ahead of time. When the player walks in, Arrived turns the booking into a live session.' },
    ...
    { to: '/loyalty', label: 'Loyalty', icon: 'star', adminOnly: true, tour: 'Top-up packages and the spend thresholds that earn a member their discount.' },
...
export function Shell({ children, onStartTour }) {
    const { session, signOut, isAdmin, cafeName, needsCafe } = useAuth();
    ...
    return (
        <div className="min-h-screen lg:flex">
            {/* Mobile top bar */}
            <div className="sticky top-0 z-30 flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 lg:hidden dark:border-slate-800 dark:bg-slate-900">
                <button
                ...
                <span className="text-sm font-semibold">CafeTrack</span>
                <span className="w-9" />
            </div>
            ...
            {/* The sidebar is fixed on desktop and a drawer on mobile. */}
            <aside
                className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-slate-200 bg-white
                            transition-transform lg:static lg:translate-x-0 dark:border-slate-800 dark:bg-slate-900
            ...
            >
                <div className="border-b border-slate-200 px-5 py-4 dark:border-slate-800">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-lg font-semibold tracking-tight">CafeTrack</span>
                    </div>
                    {/* The current cafe's name, under the logo. */}
                    <p
                        className={`mt-2 truncate text-xs ${
                            needsCafe ? 'text-amber-600 dark:text-amber-400' : 'text-slate-500 dark:text-slate-400'
                        }`}
                        title={cafeName || ''}
                    >
                        {cafeName || '—'}
                ...
                <div className="space-y-2 border-t border-slate-200 px-3 py-3 dark:border-slate-800">
                    <button
                        onClick={onStartTour}
                        className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800"
                    >
                    ...
                    <div className="rounded-lg bg-slate-50 px-3 py-2 dark:bg-slate-800/60">
                        ...
                        <Button size="sm" variant="ghost" className="mt-1 -ml-1.5" onClick={signOut}>
                            Sign out
                        </Button>
                    </div>
                </div>
    ...
    return (
        <div className="mx-auto max-w-md py-20 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                You are signed in as the platform owner, which means you do not belong to any one cafe.
                Choose one and everything else will act inside it.
            </p>
            <Link
                to="/cafes"
                className="mt-5 inline-flex items-center rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500"
            >
                Go to Cafes
            </Link>
        </div>
